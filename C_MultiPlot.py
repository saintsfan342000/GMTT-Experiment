...
for G in range(len(expts)):
    ...
    if G == len(expts)-1:
        ...
        l1 = ax11.legend([m12,m11],["Station 2", "LL"],loc='upper left',numpoints=1,fontsize=10,frameon=False)
        p.setp(l1.get_texts(),color='r')
        l2 = ff.ezlegend(ax11, loc='lower right',title='Expt; $\\alpha$; Tube', fontsize=10)
        ax11.add_artist(l1)
    ...
    if G == len(expts)-1:
        ...
        l2 = ff.ezlegend( ax21, title='Expt; $\\alpha$; Tube', markers=True)
        ...
        l2 = ff.ezlegend( ax22, title='Expt; $\\alpha$; Tube', markers=True)
    ...
    if G == len(expts) - 1:
        ...
        l3 = ff.ezlegend(ax3, loc='lower right',title='Expt; $\\alpha$; Tube',frameon=True)
    ...
    if G == len(expts) - 1:
        ...
        l3 = ff.ezlegend(ax5, loc='lower right',title='Expt; $\\alpha$; Tube',frameon=True)
        p.setp(l3.get_frame(),lw=0)
    ...
    if G == len(expts) - 1:
        ...
        ax4.set_xlim([-8,8])
        l2 = ff.ezlegend(ax4, loc='upper right',title='Expt; $\\alpha$; Tube')
    ...
    if G == len(expts) - 1:
        ...
        p.setp(leg7_2.get_lines(),mfc='r')
        leg7 = ff.ezlegend(ax7, title='Expt; $\\mathregular{\\alpha}}$; Tube', markers=True)
        # hl=0.1 is a workaround for a bug I have with the way ezlegend fails to correctly deal with points
