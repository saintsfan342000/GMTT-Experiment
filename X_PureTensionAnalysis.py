...
if not os.path.exists('box_limits.dat'):
    ...
    p.tricontourf(x,y,LEp,linspace(meanLEP-1*stdLEP,meanLEP+3*stdLEP,256),extend='both')
        ...
        if k in profStg:
            ...
            if makecontours:
                if k == last:
                    ...
                    p.tricontourf(Acol[:,2],Acol[:,3],LEp,linspace(meanLEP-1*stdLEP,meanLEP+3*stdLEP,256),extend='both')
                    #p.axis('equal')
                    p.xlabel('Undeformed X (in)')
                    p.ylabel('Undeformed Y (in)')
                    p.axis([n.min(Acol[:,2]), n.max(Acol[:,2]), n.min(Acol[:,3]), n.max(Acol[:,3])])
